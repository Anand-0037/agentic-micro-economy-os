...
**`/v1/skills` endpoint**
The Byreal Skills registry is now queryable over HTTP, MCP, and SDK. Discover which execution skills (for example `mantle.swap.v1`) an agent can invoke. See the [Byreal Skills integration guide](/guides/byreal-skills-integration).

## Improvements

**Byreal execution visibility**
Every cycle now emits a structured `byreal_skill_invocation` log line before signing and surfaces it in the replay view as **Execution · via Byreal Skills CLI**. Operators can audit *what* ran without reading worker internals.
...
The dashboard, worker, and SDK now read a consistent runtime configuration, removing drift between hosted, demo, and local deployments. See [Run locally](/guides/run-locally).
